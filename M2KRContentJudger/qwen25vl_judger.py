...
class ContentJudger:
    ...
    def judge_relevance(self, image, question, paragraph):
        """
        判断图像和OCR文本是否与问题相关
        ...
        try:
            ...
            def resize_image(t_image):
                new_h, new_w = smart_resize(t_image.height, t_image.width)
                t_image = t_image.resize((new_w, new_h))
                return t_image
            ...
            # 构建提示文本
            prompt = f"Is the following paragraph an answer to the questions raised about the image?Answer with only 'yes' or 'no'."
